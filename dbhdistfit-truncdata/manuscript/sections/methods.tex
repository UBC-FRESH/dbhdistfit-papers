...
\end{equation}
where $x_i$ is the class midpoint, $f$ the density evaluated at $x_i$, and $\bm{P}$ the free parameters. The legacy notebook weighted residuals by sampling error, but the public meta-plot summaries lack per-plot tallies, so we set $w_i = 1$ for all bins.
