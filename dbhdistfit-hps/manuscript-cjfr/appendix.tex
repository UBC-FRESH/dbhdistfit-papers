% CJFR Appendix (submitted as a separate document).
...
\title{Appendix A: Derivation of Equivalence Between Estimators}
...
\section*{Appendix A: Derivation of Equivalence Between Estimators}

This appendix presents a rigorous derivation showing that the weighted estimator introduced
in the main manuscript is equivalent to the size-biased estimator of
...
and let $\hat{y}_i$ denote the expanded stand table value
\begin{equation}
  \hat{y}_i = C_{BA} \frac{1}{|\mathcal{J}|} \sum_{j \in \mathcal{J}} t_{ij} \bar{g}_{ij}^{-1}.
\end{equation}
Define the HPS expansion factor
\begin{equation}
